...
export class MacroComponent {
  ...
  seleccionarMacro() {
    ...
    const datazonas = this.getDataTotalesZona(this.macroSelected, this.dataGeneral)
    //console.log(datazonas);
    this.graficoZonas('chartBarZonasMacro', datazonas)
  }
  ...
  getDataTotalesZona(macrodistrito: any, dataOrigen: any) {
  ...
  graficoZonas(idChart: string, data: any) {
    ...
    // Create root element
    // https://www.amcharts.com/docs/v5/getting-started/#Root_element
    let root = am5.Root.new("chartdivBarZonasMacro");
    root.locale = am5locales_es_ES;
    root._logo!.dispose();
    // Set themes
    ...
    data.forEach((zona: any) => {
      ...
      if(zona.state.includes('SOL.BO')){
        createRange(zona.region, zona.state, chart.get("colors")!.getIndex(c));
        c=c+1
      }
      //console.log(zona);
    })
    //createRange("CENTRAL", "SOL.BO (1)", chart.get("colors")!.getIndex(0));
